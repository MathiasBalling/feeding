...
_TEST_SCENE = r"""
    ...
    <worldbody>
        ...
        <body name="o1" pos="1 1 0">
            <geom name="o1" size="0.01"/>
        </body>
        <body name="o2" pos="1 1 1">
            <geom name="o2" size="0.01"/>
        </body>
...
def test_get_joint_qpos_addr(setup_scene):
    m, _ = setup_scene
    joint_name = "ur5e/elbow_joint"
    joint_qpos_addr = get_joint_qpos_addr(m, joint_name)
    assert isinstance(joint_qpos_addr, np.int32)
    assert joint_qpos_addr == 2


def test_set_joint_q(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    set_joint_q(d, m, joint_name, 1.57)
    assert d.joint(joint_name).qpos[0] == 1.57


def test_set_joint_dq(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    set_joint_dq(d, m, joint_name, 1.57)
    assert d.joint(joint_name).qvel[0] == 1.57


def test_set_joint_ddq(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    set_joint_ddq(d, m, joint_name, 1.57)
    assert d.joint(joint_name).qacc[0] == 1.57


def test_get_joint_q(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    set_joint_q(d, m, joint_name, 1.57)
    joint_q = get_joint_q(d, m, joint_name)
    assert np.all(joint_q == np.array([1.57]))


def test_get_joint_dq(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    set_joint_dq(d, m, joint_name, 1.57)
    joint_dq = get_joint_dq(d, m, joint_name)
    assert joint_dq == [1.57]


def test_get_joint_ddq(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    set_joint_ddq(d, m, joint_name, 1.57)
    joint_ddq = get_joint_ddq(d, m, joint_name)
    assert joint_ddq == [1.57]


def test_get_joint_qpos_indxs(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    qpos_indxs = get_joint_qpos_indxs(d, m, joint_name)
    assert qpos_indxs == [2]


def test_get_joint_dof_indxs(setup_scene):
    m, _ = setup_scene
    joint_name = "ur5e/elbow_joint"
    dof_indxs = get_joint_dof_indxs(m, joint_name)
    assert (dof_indxs == [2]).all()


def test_get_joint_dim(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/elbow_joint"
    joint_dim = get_joint_dim(d, m, joint_name)
    assert joint_dim == 1


def test_get_contact_states(setup_scene):
    m, d = setup_scene
    # Allow some time for contact states to be updated
    for _ in range(10):
        mj.mj_step(m, d)
    contact_states = get_contact_states(d, m)
    assert contact_states == (False, [])


def test_apply_wrench(setup_scene):
    m, d = setup_scene
    body_name = "ur5e/base"
    wrench = [1, 1, 1, 1, 1, 1]
    apply_wrench(d, m, body_name, wrench)
    # No direct output, so this is more of a state change test


def test_get_bodies_in_contact(setup_scene):
    m, d = setup_scene
    bodies_in_contact = get_bodies_in_contact(d, m)
    assert bodies_in_contact == []


def test_get_joint_pos(setup_scene):
    m, d = setup_scene
    joint_name = "ur5e/shoulder_pan_joint"
    joint_pos = get_joint_pos(d, m, joint_name)
    mj.mj_step(m, d)
    expected_pos = np.array([0.22224516, 0.37556606, 0.25090961])
    assert np.allclose(joint_pos, expected_pos)


def test_get_geom_distance(setup_scene):
    m, d = setup_scene
    mj.mj_step(m, d)
    geom_1 = "o1"
    geom_2 = "o2"
    distance, _from, _to = get_geom_distance(d, m, geom_1, geom_2)
    expected_distance = 0.98
    gt_from, gt_to = np.array([1, 1, 0.01]), np.array([1, 1, 0.99])
    assert np.isclose(distance, expected_distance)
    assert np.allclose(gt_from, _from)
    assert np.allclose(gt_to, _to)
